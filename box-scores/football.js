jQuery(document).ready(function() {
	...
	function eventStatus(){
		jQuery.ajax({
			url: "https://6thmansports.com/api/football/game/" + number,
				dataType: "json",
				cache: false,
				success: function(data) {
					...
					//  Display Possession Bubble
					if (data[0].possession === data[0].away_team_id) {
						jQuery("#home_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').addClass('text-white');
						jQuery("#away_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').removeClass('text-white');
					} else if (data[0].possession === data[0].home_team_id) {
						jQuery("#away_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').addClass('text-white');
						jQuery("#home_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').removeClass('text-white');
					} else {
						jQuery("#home_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').addClass('text-white');
						jQuery("#away_team_possession").html('<i class="fa fa-circle" aria-hidden="true"></i>').addClass('text-white');
					}
					...
					if (data[0].game_status < 1) {
						...
						if (data[0].game_status === 1) {
							...
							jQuery('#minutes').html("");
							jQuery('#seconds').html("");
							jQuery("#home_team_possession").html("");
							jQuery("#away_team_possession").html("");
							jQuery('#status').html(data[0].game_status);
